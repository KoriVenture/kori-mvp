...
const SAFE_REDIRECTS = new Set(["/onboarding/investor", "/onboarding/founder", "/profile"]);
export function safeRedirectPath(value: string | null | undefined): string {
  return value && SAFE_REDIRECTS.has(value) ? value : "/profile";
}

export function forceDeferredVerification(_value?: unknown): "deferred" {
  void _value;
  return "deferred";
}

export const documentTypes = ["pitch_deck", "company_overview", "supporting_document"] as const;
...
export const agreementsSchema = z.object({
  terms: z.literal(true), privacy: z.literal(true), platform: z.literal(true),
  risk: z.boolean(), signatureName: z.string().trim().min(2).max(200),
